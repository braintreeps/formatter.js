...
Formatter.prototype._processKey = function (chars, delKey,ingoreCaret) {
  ...
  if (delKey) {
    ...
    console.log(this.val, this.sel.begin, this.sel.end, this.chars, this.chars[this.sel.end-1]);
    for (var i = this.sel.end-1; i > 0 && this.chars[i]; i--) {
      this.delta--;
    }
  }
  console.log('extra delta', this.delta);
  ...
  // or Backspace and not at start
    ...
    this.delta -= 1;
    console.log("before remove: ", this.val);
    this.val = utils.removeChars(this.val, this.sel.end + this.delta, this.sel.end);
    console.log("after remove: ", this.val);
  ...
  console.log("before format: ", this.val);

  // Format el.value (also handles updating caret position)
  this._formatValue(ingoreCaret);

  console.log("after format: ", this.val);
};
...
Formatter.prototype._removeChars = function () {
  ...
  for (var i = 0; i <= this.mLength; i++) {
    ...
    // console.log("Before removeChars", { "this.val": this.val, "val": val, "curChar": curChar, "curHldr": curHldr, "pos": pos });
    // Remove char and account for shift
    if (curChar && curChar == val || curHldr && curHldr == val) {
      this.val = utils.removeChars(this.val, pos, pos + 1);
      shift--;
    }
    // console.log("After removeChars", { "this.val": this.val });
  // console.log();
  }
  // console.log("----");
